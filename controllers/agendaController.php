...
class agendaController extends controller {
    ...
    public function index($page = 1) {
        $view = "noticias/agenda";
        $dados = array();
        $crud = new crud_db();
        $dados = array();
        $dados['genero'] = 'feed';
        $arraySearch = array();
        $sql = "SELECT p.*, c.nome as categoria, u.nome FROM post AS p INNER JOIN categoria as c INNER JOIN usuario as u WHERE p.id_categoria=c.id AND  p.id_usuario=u.id AND p.status=1 AND p.genero='agenda'";
        $parametros = "";
